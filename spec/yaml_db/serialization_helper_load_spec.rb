  ...
  module SerializationHelper
    RSpec.describe Load do
      ...
      it "truncates the table" do
        allow(ActiveRecord::Base.connection).to receive(:execute).with("SAVEPOINT before_truncation")
        allow(ActiveRecord::Base.connection).to receive(:execute).with("TRUNCATE mytable CASCADE").and_return(true)
      ...
      it "deletes the table if truncate throws an exception" do
        expect(ActiveRecord::Base.connection).to receive(:execute).with("SAVEPOINT before_truncation")
        expect(ActiveRecord::Base.connection).to receive(:execute).with("TRUNCATE mytable CASCADE").and_raise()
        expect(ActiveRecord::Base.connection).to receive(:execute).with("ROLLBACK TO SAVEPOINT before_truncation")
